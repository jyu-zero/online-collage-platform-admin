            ...
            setTimeout(() => {
                if(this.isMoveImg){
                    ...
                    if(y < (parentEle.offsetHeight - selectingCoverEle.offsetHeight) && y >= 0){
                        selectingPreEle.style.top = parseInt(selectingPreEle.style.top) + y + 'px'
                        selectingCoverEle.style.top = parseInt(selectingCoverEle.style.top) + y + 'px'
                        selectingPreImgEle.style.top = parseInt(selectingPreImgEle.style.top) + (-y) + 'px'
                        showSelectingimgEle.style.top = parseInt(showSelectingimgEle.style.top) + (-y) + 'px'
                    }
                    if(x < (parentEle.offsetWidth - selectingCoverEle.offsetWidth) && x >= 0){
                        selectingPreEle.style.left = parseInt(selectingPreEle.style.left) + x + 'px'
                        selectingCoverEle.style.left = parseInt(selectingCoverEle.style.left) + x + 'px'
                        selectingPreImgEle.style.left = parseInt(selectingPreImgEle.style.left) + (-x) + 'px'
                        showSelectingimgEle.style.left = parseInt(showSelectingimgEle.style.left) + (-x) + 'px'
                    }
                    this.divPos = pos
                    console.log(selectingCoverEle.offsetWidth, y)
                }
            }, 25)
        },
        beginMoveImg(event){
            ...
            if(this.isFirstMoveImg){
                ...
                let selectingCoverEle = document.getElementById('selecting-cover')
                this.divPos = this.getPos(selectingCoverEle.parentElement, event)
            }
        },
        endMoveImg(){

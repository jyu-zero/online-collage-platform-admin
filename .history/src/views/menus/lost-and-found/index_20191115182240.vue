                            ...
                            <div class="lost-upload">
                                <div class="upload-content"
                                    ...
                                    <div class="select-size"
                                         ...
                                         :style="{'width': divSize.width + 'px', 'height': divSize.height + 'px',
                                         'left': imgPosition.left + 'px', 'top': imgPosition.top + 'px'}">
                                        <span id="top-left-btn" @mousedown="beginSetDivSize" @mousemove="setDivSize($event,'top-left')"></span>
                                        <span id="top-right-btn" @mousedown="beginSetDivSize" @mousemove="setDivSize($event,'top-right')"></span>
                                        <span id="bottom-left-btn" @mousedown="beginSetDivSize" @mousemove="setDivSize($event,'bottom-left')"></span>
                                        <span id="bottom-right-btn" @mousedown="beginSetDivSize" @mousemove="setDivSize($event,'bottom-right')"></span>
                                    </div>
                                </div>
                                <div class="line"></div>
                                <div class="show-wrap">
        ...
        moveSelectImg(event){
            setTimeout(() => {
                if(this.isMoveImg){
                    ...
                    if(gapLeft < 0){
                        x = 0
                    }else if(gapWidth < gapLeft){
                        x = gapWidth
                    }

                    if(gapTop < 0){
                        y = 0
                    }else if(gapHeight < gapTop){
                        y = gapHeight
                    }
        ...
        setDivSize(event, dir){
            if(this.isDivImgSize){
                ...
                setTimeout(() => {
                    ...
                    let gapLeft = selectingCoverEle.offsetLeft
                    let gapTop = selectingCoverEle.offsetTop
                    let gapWidth = parentEle.scrollWidth - selectingCoverEle.scrollWidth
                    let gapHeight = parentEle.scrollHeight - selectingCoverEle.scrollHeight
                    let x, y
                    let isSetSize = true

                    if(gapWidth <= gapLeft + 1 || gapLeft < 0){
                        isSetSize = false
                    }
                    if(gapHeight <= gapTop + 1 || gapTop < 0){
                        isSetSize = false
                    }
                    console.log(gapTop, gapHeight)
                    if(isSetSize){
                        switch(dir){
                            case 'top-left':
